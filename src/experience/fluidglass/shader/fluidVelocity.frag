...
void main() {
    ...
    vec2 pressureAcceleration = -gradient * pressureStrength / (center + 0.001);
    float fluidMass = smoothstep(0.0, 0.85, center);
    vec2 gravityAcceleration = gravity * fluidMass;
    ...
    velocity += pressureAcceleration + diffusion * viscosity + flow * flowStrength;
    velocity += gravityAcceleration;
    velocity *= 1.0 - drag;

    // A soft floor catches falling liquid while preserving sideways spreading.
    float floorInfluence = 1.0 - smoothstep(0.0, floorLevel, vUv.y);
    velocity.y = mix(velocity.y, max(velocity.y, 0.0), floorInfluence * floorResistance);
